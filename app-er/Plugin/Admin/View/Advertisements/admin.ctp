            ...
            <div class="content">
                <table cellpadding="0" cellspacing="0" width="100%" class="table table-bordered table-striped"
                    ...
                    <?php foreach ($ads as $key => $value) {
                        $id = $value["Advertise"]["id"]; ?>
                        <tr>

                            <td><?php $image = '/files/ads/'.$id.'/'.$value['Advertise']['image'];
                                echo $this->Html->image($image, array('class' =>'img-responsive img-circle', 'style'=>'height:50px')); ?></td>
                            <td><?php echo $value['Advertise']['name']; ?></td>
                            <td><?php echo $value['Advertise']['description']; ?></td>
                            <td><?php echo $value['AdvertiseCategory']['name']; ?></td>
                            <td>
                                <?php $status = $value['Advertise']['status'];
                                if ($status) {
                                    echo $this->Html->link('<i class="fa fa-check-circle"></i>',
                                        array('controller' => "advertisements", "action" => "change_visible_status", $id, 0,
                                            'plugin' =>'admin'),
                                        array("class" => "text-success", "escape" => false, "title" => __("Disable It.")));
                                } else {
                                    echo $this->Html->link('<i class="fa fa-minus-circle"></i>',
                                        array('controller' => "advertisements", "action" => "change_visible_status", $id, true,
                                            'plugin' =>'admin'),
                            ...
                            <td>
                                ...
                                $url = array("controller" => "advertisements", "action" => "update", $id, "plugin" => 'admin');
                                ...
                                if (empty($isDeleted)) {
                                    ...
                                    echo $this->Form->postLink(
                                        $removeText,
                                        array("controller" => "advertisements", 'action' => 'remove', $id, 'plugin' =>'admin'),
                                        array('confirm' => 'Are you sure to remove it?', "class" => "text-danger", "escape" => false,
                                            "title" => __("Remove Advertise Type"))
                                    );
                                }
                                ?>
                            </td>
